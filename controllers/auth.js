...
const login = async (req = request, res = response) => {
    ...
    try {
        ...
        if (!usuario.estado) return res.status(400).json({ msg: 'La cuenta del usuario esta inactivo' });

        const validarPassword = bcryptjs.compareSync(password, usuario.password);
        if (!validarPassword) return res.status(400).json({ msg: 'La password es incorrecta' });
